...
class Respiro : public QAbstractListModel
{
	...
	Q_PROPERTY(float		CO2ADC				READ CO2ADC				WRITE setCO2ADC						NOTIFY CO2ADCChanged			)
	Q_PROPERTY(float		flow				READ flow				WRITE setFlow						NOTIFY flowChanged				)
	...
	Q_PROPERTY(bool			pumpBypass			READ pumpBypass			WRITE setPumpBypass					NOTIFY pumpBypassChanged		)
	...
	Q_PROPERTY(int			runtimeTotalSec		READ runtimeTotalSec	WRITE setRuntimeTotalSec			NOTIFY runtimeTotalSecChanged	)
...
public:
  explicit				Respiro();
	...
	float CO2ADC() const;
	void setCO2ADC(float newCO2ADC);
	
	float flow() const;
	void setFlow(float newFlow);
	
	bool pumpBypass() const;
	void setPumpBypass(bool newPumpBypass);
	
	int runtimeTotalSec() const;
	void setRuntimeTotalSec(int newRuntimeTotalSec);
	
public slots:
	void				setChannelInit(		int index, bool checked);
	void				updateChannelConfig(int channelID, QString sampleID, double hsVol_ml,
						double CO2max_ppm, double O2min_perc, double O2max_perc, double CH4max_ppm);
	void				updateChannelStatus(int channelID, bool leakPass, bool pressPass,
						bool volPass, bool measureStable, bool isActive, bool inExperiment);
	void				updateChannelStatusText(int channelID, QString statusText);
	void				updateChannelColor(int channelID, QString color);
	void				updateChannelRuntime(int channelID, double cycle, double startTime, double completeCycle);
	void					push_meas_data();
	void					push_proc_data();
	...
	void					measure(int c);
	void					flush(int c);
	void					openedLid(int c);
	void					measureHeadspacePost(int c);
	void					sendPumpOn(bool on);
	void					sendVent0(bool open);
	void					sendVent1(bool open);
	void					sendVent2(bool open);
	void					sendPumpBypass(bool bypass);
	void					init();
...
signals:
	//void					directDrive(QString command): ?
	void					basalState(int s);
	void					O2Changed();
	void					channelConfigUpdated(int channelID, QString sampleID, double hsVol_ml, double CO2max_ppm, double O2min_perc, double O2max_perc, double CH4max_ppm);
	...
	void					flushSignal(int c);
	void					openedLidSignal(int c);
	void					measureHeadspacePostSignal(int c);
	void					pumpOnSignal(bool on);
	void					vent0Signal(bool open);
	void					vent1Signal(bool open);
	void					vent2Signal(bool open);
	void					pumpBypassSignal(bool bypass);
	...
	void					CO2ADCChanged();
	void					flowChanged();
	void					pumpBypassChanged();
	
	void runtimeTotalSecChanged();
